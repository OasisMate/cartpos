...
- [x] **Shared product catalog (new-shop onboarding)** - BUILT 2026-08-24 (`feat/starter-catalogs`, migration NOT yet applied). A shop signing up has no data and nobody hand-types 2,000 items; the market expects a POS usable on day one. New global `CatalogProduct` + `CatalogSighting` tables (barcode-keyed, RLS on). Picker modal on the products page: browse by category, search, tick, set price inline, bulk add. Seeded from Rose Mart via `build-master-catalog.ts` (clean + auto-categorize -> reviewable CSV) then `seed-master-catalog.ts`. **Self-updating:** every barcoded product any shop adds is captured as a sighting; 2 independent shops promote it to visible, and suggestedPrice is the median across sightings. Cost/trade prices are never collected. `catalog.ts`, `catalog-taxonomy.ts` (Pakistani FMCG brand rules, 83% coverage), `catalog-import.ts`, `/api/catalog/*`.
...
- [x] Session revoke on password reset/change (`tokenVersion`) — DONE 2026-08-27, live-verified. Free on the hot path: the version rides on the user row `getCurrentUser` already reads.
- [ ] Reset-token email-binding. ← NEXT
