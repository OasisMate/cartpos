...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const getNavGroups = (): NavGroup[] => {
    ...
    //
    // Only for plans that include the organisation layer. On Solo and Team the
    // owner runs a single shop, so they get the STORE navigation instead by
    // falling through to the store-manager branch below, exactly as intended:
    // "treat that user as a shop manager, no org level access".
    //
    // They remain ORG_ADMIN in the database. Hiding this surface changes no
    // permissions, which is why a downgrade is reversible.
    const showOrgLevel = user?.billing?.allowOrgLevel !== false

    if (isOrgAdmin && user?.currentOrgId && showOrgLevel) {
