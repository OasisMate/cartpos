...
interface OrgUsersViewProps {
  // Set only by the platform-admin route /org/[orgId]/users, which knows the org
  // from the URL. The org-admin route /org/users leaves these undefined and the
  // API falls back to the currentOrgId cookie, which is that user's only org.
  orgId?: string
  orgName?: string
}

export default function OrgUsersView({ orgId, orgName }: OrgUsersViewProps) {
  const router = useRouter()
  const { user } = useAuth()
  // Pin every call to the org in the URL. Without this a stale currentOrgId
  // cookie (another tab, a pasted link) lists the wrong org's staff here.
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const res = await fetch(`/api/org/users${scope}`)
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-bold mb-2 bg-gradient-to-r from-blue-600 to-orange-600 bg-clip-text text-transparent">
            {orgName ? `Users in ${orgName}` : 'Users Management'}
          </h1>
          <p className="text-gray-600">
            Staff of this organization and their store assignments
          </p>
        </div>
        {!showForm && (
          <button
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center justify-end gap-2">
                        <IconButton
                          ...
                          label="Edit user"
                          onClick={() => {
                            router.push(orgId ? `/org/${orgId}/users/${u.id}` : `/org/users/${u.id}`)
                          }}
                        >
